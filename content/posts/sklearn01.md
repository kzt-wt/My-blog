...
## 背景
[House Priceコンペ](https://www.kaggle.com/c/house-prices-advanced-regression-techniques)で一番デバッグに苦戦したことを共有します。

実際、一番苦戦したのがこのエンコーディングです。
今までエンコーディングをあまく見ていました...
...
## 正しいエンコーディング
先ほどとは違い、訓練用データ＋テスト用データを結合してからの*エンコーディング*でいきます
``` python
...
## 原因
原因は検証・テスト用データで分けた状態で`pd.get_dummies`を使っていたから。

検証用とテスト用でカラム数が変わってしまったため`pd.get_dummies`に怒られたようです。
